...
- [x] 7-1: `./gradlew clean build` 通過(全テスト)
- [x] 7-2: WAR 起動 + 実機フロー検証 29 項目 OK: ブートストラップ管理者でログイン → 登録申請(MailPit でメール確認・リンク取得)→ パスワード設定 → 承認(メール確認)→ 新規ユーザでログイン → リフレッシュ → 再利用検知(即時アラートメール)→ ログアウト。ロックアウト(失敗 5 回 → 423)・解除・アラートメール(閾値超過)も確認
- [x] 7-3: セキュリティヘッダー実測(CSP ほか、SPA/API 両面)+ 監査ログ内容確認(audit_log 12 種別・detail はコードのみ)+ トークンはハッシュのみ保存・パスワードは BCrypt を確認
- [x] 7-4: US-047/048 実機確認(言語・テーマのサーバ保存 → 別セッションのログイン応答・GET /api/me で復元)
- [x] 7-5: code-summary.md 作成
