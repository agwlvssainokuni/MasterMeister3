...
- [x] Step 1: Functional Design 成果物の分析(暗号化・プール・取込規模・キャッシュ・YAML の NFR 抽出 — NFR-U4-01〜08)
- [x] Step 2: 質問(Q1〜Q3)の回答確定(B / A / A)
- [x] Step 3: `mm.app.credential.*` ほか設定プロパティ体系の確定(nfr-requirements.md §2 — 4 プロパティ・起動時検証)
- [x] Step 4: 追加依存の確定と成果物生成(全て Boot BOM 管理 — tech-stack-decisions.md。nfr-requirements.md / tech-stack-decisions.md 生成済み)
...
[Answer]: B(各公式ドライバ。MySQL Connector/J は GPLv2 + FOSS 例外 — 同梱可の判断を tech-stack-decisions に記録)
...
[Answer]: A(jackson-dataformat-yaml — Jackson 3 系列に統一)
...
[Answer]: A(サイズ上限 + 防御的 TTL 10 分・設定可能)
