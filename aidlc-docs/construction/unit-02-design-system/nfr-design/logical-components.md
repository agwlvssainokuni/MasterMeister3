...
**③への申し送り**:
1. テーマ・言語のユーザ設定(サーバ保存)との統合(localStorage は初期表示用に降格)
2. H-07: 監査イベントのロケール非依存記録(バックエンド側)

(FOUC 防止は外部ファイル方式(public/theme-init.js)に確定したため、CSP 特例の申し送りは解消 — `script-src 'self'` で完結)
