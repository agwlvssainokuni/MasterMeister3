...
| 配置 | 実装 |
|---|---|
| app | apiClient(Bearer・401 シングルフライトリフレッシュ・Problem Details 解釈)、tokenStore(sessionStorage 集約 — apiClient と対の基盤資産)、AppLayout(AppShell 統合・ナビ・ユーザメニュー)、HomePage(プレースホルダ)、routes、standalone.module.css(未ログイン単独画面の共通レイアウト) |
| features/auth | api.ts(login/logout/me/preferences)、AuthProvider(復元・ログイン・ログアウト・サーバ設定適用)、guards(RequireAuth/RequireAdmin)、usePreferences(サーバ保存)、LoginPage |
