...
| NFR-U4-05 | 取込の性能・堅牢性 | 設計ガイド: 接続あたり 2,000 テーブル / 50,000 カラム規模を単一トランザクションで取込完了。保存はシンプルな saveAll(**バッチ最適化なし** — NFR Design Q4=C で改訂: IDENTITY 採番のため Hibernate バッチは無効・内部 H2 は同一プロセス。実測で問題時に SEQUENCE + バッチ検討)。失敗時は全ロールバック(取込前状態を維持) | 結合テスト(取込 → 再取込 → 途中失敗ロールバック)。大規模実測は Build and Test ステージ判断 |
| NFR-U4-06 | 権限解決の性能 | キャッシュミス時でも 1 接続分の全カラム解決を 1 秒以内(エントリ・グループ・メタデータを各 1 クエリで取得しメモリ上で解決 — N+1 禁止) | 解決ロジックの単体テスト + クエリ数の検証 |
| NFR-U4-07 | テスト方針 | 単体・結合テストの主体は**ターゲットも H2**(db_type=H2 の接続を登録)。方言依存部(実接続・スキーマ取込・DbDialect の URL/スキーマ概念)は**①導入済みの Testcontainers 実エンジンテスト**(MySQL/MariaDB/PostgreSQL、Docker なし環境は自動 skip — ①のスモークテスト基盤を拡張)で自動化。devenv は開発時の常設環境 + DoD 実機フロー検証に使用(§3 判断メモ) | ./gradlew build で全テスト(Docker あり環境では実エンジン含む)+ DoD 実機フロー |
...
- **Testcontainers と devenv の役割分担**(①の決定 Q3=A を踏襲 — 当初記載「不採用」はユーザ指摘により訂正): Testcontainers = 自動結合テスト(使い捨てコンテナで状態汚染なし・①の実エンジンスモークを④で取込テストに拡張)、devenv = 開発時の常設環境 + DoD 実機フロー検証。方言非依存の業務ロジックは H2 ターゲットで網羅する
